...
::: fragment
- Theoretical foundation for mechanism
:::
...
::: fragment
- External validity through both a survey and past RCTs
:::
...
- **Hypothesis 2**: Sampling and context precision are
    - DARA: Complements
    - CARA: Independent
    - IARA: Substitutes
:::
...
- 1,571 small and marginal farmers
- Participant must decide how intensively to adopt a hypothetical technology
- Pay based on crop yield from decision (~1/2 days wage)
:::
...
![](game_5.png){width=350px}


![](game_14.png){width=350px}
...
![](game_13.png){width=350px}


![](game_15.png){width=350px}
...
survey_df |>
  ...
  mutate(name = fct_relevel(name, rev(c('Different Soil', 'Different Access To Inputs', 'Poor Tech QC', 'I Lack Training', 'Not Enough Seasons', 'They Lack Knowledge', 'Ulterior Motives',    'Technology Unavailable',  'Lack Capital')))) |>
  group_by(name) |>
...
# Heterogeneity Concerns Correlate With Aversion

``` {r echo=FALSE, dev.args = list(bg = 'transparent'), fig.retina=1, strip.white=TRUE, fig.asp=0.7,fig.align='center'}
library(dplyr)

surv_df <- haven::read_dta('/Volumes/Secomba/halidaee/Boxcryptor/Dropbox/OFTAD_encrypted/dta/cleaned_merged.dta')

df_clean.long <-  read_csv('/users/halidaee/Dropbox/Ongoing Research/Context/DataSync/current_choicedata_clean_long.csv')



df_clean.wide <- df_clean.long %>%
  pivot_wider(id_cols = c(EnumeratorID, UserID, LastTimeSaved), names_from=Map, values_from=Decision, names_prefix='Map_') 
  
df_clean.wide2 <- df_clean.long %>%
  group_by(EnumeratorID, UserID, LastTimeSaved) %>%
  pivot_wider(id_cols = c(EnumeratorID, UserID, LastTimeSaved), names_from=Module, values_from=ModuleFlip, names_prefix='ModuleFlip_', values_fn = ~as.logical(sum(.x, na.rm=T)/2)) %>%
  ungroup()

df_clean.wide <- df_clean.wide %>%
                  left_join(df_clean.wide2, by=c('UserID','LastTimeSaved','EnumeratorID'))


df_clean.attributes <- df_clean.wide %>% 
  mutate(UserID = as.factor(UserID)) %>%
  left_join(surv_df %>% mutate(hhid = as.factor(hhid)), by=c('UserID' = 'hhid')) 



reg_output <- df_clean.attributes %>%
  filter(!(is.na(c05_trust)) & c05_trust != "") |>
  mutate(across(starts_with('c05_trust_'), as.logical)) %>%
  mutate(hetero_trust = pmax(c05_trust_1, c05_trust_2, c05_trust_6, c05_trust_8)) %>%
  mutate(difference = Map_13 - Map_12) %>% 
  lm(difference ~  c05_trust_1+ c05_trust_2+ c05_trust_6+ c05_trust_8  , data=.) %>% 
  tidy()

reg_output <- reg_output[-1,]
reg_output$term <- c('Different Soil', 'Different Input \n Access', 'Poor Tech QC', 'I Lack Training')
...
reg_output %>%   
  mutate(conf.low = estimate - 1.96*std.error, 
         ...
         theme(axis.line=element_line(colour = '#0a3b4c'),
          plot.title = element_text(size=15,face='bold'),
          axis.text.x=element_text(size=20, color = '#0a3b4c'),
          axis.text.y=element_text(size=20, color = '#0a3b4c'),
          #axis.ticks=element_blank(),
          legend.position="none",
          panel.background=element_blank(),
          #panel.border=element_blank(),
